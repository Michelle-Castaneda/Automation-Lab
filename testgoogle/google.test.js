...
//   await search.sendKeys('First Search', Key.RETURN);
...
//   search = await driver.findElement(By.name('q'));
//   await search.clear();
//   // TODO Call .sendKeys() on the search bar element to search for a new term
//   await driver.sendKeys("Second Search", Key.RETURN);
...
test("can search Google twice", async () => {
  ...
  await driver.get("https://www.google.com/");
  // TODO Search for something in Google and wait for the page to load
  await driver.findElement(By.name("q")).sendKeys("Devmountain", Key.RETURN);
  // TODO Call .clear() on the search bar element to clear the old search term
  await driver.findElement(By.name("q")).clear();
  // TODO Call .sendKeys() on the search bar element to search for a new term
  await driver.findElement(By.name('q')).sendKeys('Perficient', Key.RETURN)
  // TODO Wait for the results page to load
  await driver.wait(until.titleIs('Perficient - Google Search'), 1000)
});